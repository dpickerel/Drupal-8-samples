...
/**
 * Susquehanna specific form for accepting hospital payments
 */
class DonorHospitalPaymentForm extends FormBase
{
    /**
     * {@inheritdoc}
     */
    ...
    {
        return 'hospital_payment_block_form';
    }
    ...
    $form['patient'] = array(
      ...
      '#tree' => TRUE,
        ...
        $form['patient']['pname'] = array(
            '#type' => 'textfield',
            '#title' => $this->t('1. Patient Name:'),
            '#required' => TRUE,
        );
        $form['patient']['stmt'] = array(
            '#type' => 'textfield',
            '#title' => $this->t('2. Statement Number or Letter Number:'),
            '#required' => TRUE,
        );

        $form['patient']['payment'] = array(
            '#type' => 'textfield',
            '#title' => $this->t('3. Payment Amount:'),
            '#required' => TRUE,
        );
    ...
    $form['ccinfo'] = array(
      ...
      $form['ccinfo']['cvv'] = array(
        ...
        $form['ccinfo']['expires'] = array(
            '#markup' => t('Expiration Date:'),
        );
    ...
    public function validateForm(array &$form, FormStateInterface $form_state) {
        ...
        module_load_include('php', 'donor_forms', 'lib/ccvalidate');
          ...
          if(empty($form_state->getValue('patient')['payment'])){
             $form_state->setErrorByName('patient][payment', t('Please specify a payment amount.'));
          };        
          ...
          if(empty($form_state->getValue('ccinfo')['cvv'])){
             $form_state->setErrorByName('ccinfo][cvv', t('Please enter a valid cvv number.'));
          };
          if(drupal_forms_validate_cvv($form_state->getValue('ccinfo')['ccno'],$form_state->getValue('ccinfo')['cvv']) === FALSE){
             $form_state->setErrorByName('ccinfo][cvv', t('The cvv number entered is not valid for this card.'));
          }
    ...
    $guarantor_array = $form_state->getValues();
     $charge = $form_state->getValue('patient')['payment'];
    ...
    $nvp = array(
      ...
      'TENDER'  => 'C',
      // The total price
      'AMT'     =>  $form_state->getValue('patient')['payment'],
      // The currency code
      'CURRENCY' => 'USD',
    ...
    $new_page_values['field_confirmation_code'] = drupal_forms_payflow_pro_submit($nvp, $charge); 
          

    $new_page_values['body'] = '';
    $new_page_values['type'] = 'hospital_payment';
    $new_page_values['title'] = $form_state->getValue('billing')['first'] . ' ' . $form_state->getValue('billing')['last'] ;

    $new_page_values['field_patient_name'] = $form_state->getValue('patient')['pname'];
    $new_page_values['field_statement_number'] = $form_state->getValue('patient')['stmt'];
    $new_page_values['field_hospital_payment_amount'] = $form_state->getValue('patient')['payment'];

    $new_page_values['field_hospital_first_name'] = $form_state->getValue('billing')['first'];
    $new_page_values['field_hospital_last_name'] = $form_state->getValue('billing')['last'];
    $new_page_values['field_hospital_payment_email'] = $form_state->getValue('billing')['last'];
    
    $new_page_values['field_hospital_address'] = $form_state->getValue('billing')['address'];
    $new_page_values['field_hospital_address_2'] = $form_state->getValue('billing')['address2'];
    $new_page_values['field_h'] = $form_state->getValue('billing')['city'];
    $new_page_values['field_hospital_state'] = $form_state->getValue('billing')['state'];
    $new_page_values['field_ho'] = $form_state->getValue('billing')['zip'];
    $new_page_values['field_hospital_phone'] = $form_state->getValue('billing')['phone'];
    ...
    private function donor_admin_mail($info){
        ...
        $params['message'] = 'A hospital payment has just been processed. You can see all payments at ' . $base_url . 'admin/reports/hospital-payments';
